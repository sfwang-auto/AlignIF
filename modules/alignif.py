...
class AlignIF(nn.Module):
    ...
    def msa_encode(self, data):
        ...
        # average merge
        # weights = torch.cat([torch.ones(1, bsz), torch.rand(self.n_aligns, bsz)], dim=0).to(device)
        weights = torch.rand((self.n_aligns + 1, bsz), device=device)
    ...
    def forward(self, data):
        h_V, h_E = self.msa_encode(data)

        for layer in self.decoder_layers:
            h_V = layer(h_V, h_E, data[0].edge_index)
        
        logits = self.W_out(h_V)
        ce_loss = self.cal_ce_loss(data[0].seq, data[0].mask, logits)
        return ce_loss
