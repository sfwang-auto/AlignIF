...
class MixUpDataset(data.Dataset):
    ...
    def process(self, id, align_id=None):
        ...
        aligned_coords = None
        if align_id is not None:
            align = data['align'][align_id]
            aligned_coords = data['aligned_coords'][align_id][:, self.bb_idx].astype(np.float32)
        ...
        return Data(
            ...
            num_nodes=num_nodes, edge_index=edge_index, 
            coords=coords, central_coords=central_coords, 
            align=[align], aligned_coords=[aligned_coords], 
        )
    ...
    def __getitem__(self, idx): 
        ...
        if self.is_test or len(align_ids) == 0:
            data_list = [data, data]
        else:
            random.shuffle(align_ids)
            data_list = [data, self.process(align_ids[0], id)]
